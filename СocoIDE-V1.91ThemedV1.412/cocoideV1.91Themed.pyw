...
def smartWriterOfTheme(colorOfBgS, colorOfBgBanksS, colorOfTextLeftBgS, colorOfBordersS, ChangedBankS, markColorS, fontColorS, colorOfCommandsS, colorOfMarksS, colorOfOperatorsS, lastOpened):
        fileTheme = open ('Themes\current.txt', 'w')
        fileTheme.write("Цвета текущей темы\n")
        fileTheme.write("Цвет заднего фона - " + colorOfBgS + "\n")
        fileTheme.write("Цвет фона ячеек памяти - " + colorOfBgBanksS + "\n")
        fileTheme.write("Цвет боковой плашки текстового окна - " + colorOfTextLeftBgS + "\n")
        fileTheme.write("Цвет границ - " + colorOfBordersS + "\n")
        fileTheme.write("Цвет изменения ячейки - " + ChangedBankS + "\n")
        fileTheme.write("Цвет шрифта ошибки - " + markColorS + "\n")
        fileTheme.write("Цвет шрифта - " + fontColorS + "\n")
        fileTheme.write("Подсветка синтаксиса команд - " + colorOfCommandsS + "\n")
        fileTheme.write("Подсветка синтаксиса меток - " + colorOfMarksS + "\n")
        fileTheme.write("Подсветка синтаксиса операторов - " + colorOfOperatorsS + "\n")
        fileTheme.write("Последний открытый файл - \\" + lastOpened + "\n")
        fileTheme.close()

def smartReaderOfTheme(path):
        fileTheme = open (path, 'r')
        #Проверяем на наличие файла с темами:
        if os.path.exists(path) and os.path.getsize(path) > 0:
            ...
            #Считывание пути последнего открытого файла
            NoLast = 1
            while(char != '\\'):
                if(char == ''):
                    NoLast = 0
            ...
            if(NoLast): 
                while(char !=''):
                    ...
                    #if(char == '\\'): char = '/'
                    if(char == '\n') : break
                    lastOpendedFile += char
            #Если нет, то путь пустой
            else: lastOpendedFile = ''
            #print(colorOfBg , colorOfBgBanks , colorOfTextLeftBg , colorOfBorders , ChangedBank , markColor , fontColor , colorOfCommands , colorOfMarks , colorOfOperators,"DADA1", lastOpendedFile +'\n')
            fileTheme.close()
        #Если нет, то создаём сами, с тёмной темой + рестарт
        else:
           smartWriterOfTheme('#35333D' , '#465362' , '#393740' , '#28262D' , '#DC5EAB' , '#8862CB' , '#D8D8D8' , '#32A5F2' , '#0BA242' , '#B232F2')
           fileTheme.close()
...
try:
    smartReaderOfTheme('Themes\current.txt')
except colorOfBg == "":
    smartWriterOfTheme('#35333D' , '#465362' , '#393740' , '#28262D' , '#DC5EAB' , '#8862CB' , '#D8D8D8' , '#32A5F2' , '#0BA242' , '#B232F2', '')
    python = sys.executable
    os.execl(python, python, * sys.argv)
...
#print(colorOfBg , colorOfBgBanks , colorOfTextLeftBg , colorOfBorders , ChangedBank , markColor , fontColor , colorOfCommands , colorOfMarks , colorOfOperators,"DADA", lastOpendedFile + '\n')
#themeFile = open ('themes\current.txt', 'r')
#colorOfBg = str(themeFile.read(7))
#colorOfBgBanks = str(themeFile.read(7))
#colorOfTextLeftBg = str(themeFile.read(7))
#colorOfBorders = str(themeFile.read(7))
#ChangedBank = str(themeFile.read(7))
#markColor = str(themeFile.read(7))
#fontColor = str(themeFile.read(7))
#themeFile.close()

#Цвета идут в файле в порядке:
#colorOfBg = '#35333D' #35333D
#colorOfBgBanks = '#465362'
#colorOfTextLeftBg = '#393740'
#colorOfBorders = '#28262D'
#ChangedBank = 'pink'
#markColor = '#8862CB'
#fontColor = '#D8D8D8'


#Создание цветов для NoteBook
myColor1 = colorOfBg
myColor2 = colorOfBgBanks

style = ttk.Style()


style.theme_create("SpecialColors", parent="alt", settings={
        "TNotebook": {"background" : "red"},
        "TNotebook.Tab": {
            "configure": {"padding": [5, 1], "background": myColor1 },
            "map":       {"background": [("selected", myColor2)],
                          "expand": [("selected", [1, 1, 1, 0])] } } 
   
        } )        
style.theme_settings("SpecialColors", {
   "TScrollbar": {
       "configure": {"width": 5, "height" : 5},
       "map": {
           "background": "red",
       }
   }
    ...
    * This frame only allows vertical scrolling
    ...
    def __init__(self, parent, *args, **kw):
        ...
        canvas = tk.Canvas(self, bd=0, highlightthickness=0,
                        yscrollcommand=vscrollbar.set, bg=colorOfBg)
...
class CocoIDE(tk.Frame):
    ...
    def __init__(self, Emulator=None, master=None, filename=None, name='cocoide'):
        ...
        tk.Frame.__init__(self, master=master, name=name, bg=colorOfBg)
        ...
        self.filemenu = tk.Menu(self.menubar, tearoff=0, bg = colorOfBg, fg = fontColor, bd = -1)
        ...
        # Edit menu
        self.editmenu = tk.Menu(self.menubar, tearoff=0, bg = colorOfBg, fg = fontColor, bd = -1)
        ...
        self.txtmenu = tk.Menu(self.editmenu, tearoff=0, bg = colorOfBg, fg = fontColor, bd = -1)
        ...
        # Emulator menu
        self.emumenu = tk.Menu(self.menubar, tearoff=0, bg = colorOfBg, fg = fontColor, bd = -1)
        ...
        #Менюшка с темами
        self.thememenu = tk.Menu(self.menubar, tearoff=0, bg = colorOfBg, fg = fontColor, bd = -1)
        self.thememenu.add_command(label="Dark Aquatic", command=self.themeDark)
        self.thememenu.add_command(label="Light Spirit", command=self.themeLight)
        self.thememenu.add_command(label="Extra Theme", command=self.themePink)
        self.thememenu.add_command(label="My theme", command=self.myThemeCreate)
        self.thememenu.add_separator()
        self.thememenu.add_command(label = "Made by Babenko")
        self.menubar.add_cascade(label="Themes", menu=self.thememenu)
        
        # Help Menu
        self.helpmenu = tk.Menu(self.menubar, tearoff=0, bg = colorOfBg, fg = fontColor, bd = -1)
        ...
        ### Buttonbar
        buttonBar = tk.Frame(self, name="buttonbar", height=40, bg = colorOfBg)#, border=1)#, bg="red")
        buttonBar.grid(row=0,column=0, columnspan =4, sticky="ew")

        ## Shortcut buttons
        # Editor
        self.editButtons = tk.Frame(buttonBar, background=colorOfBg)
        self.editButtons.pack(side=tk.LEFT, fill=tk.BOTH)
        self.newButton = tk.Button(self.editButtons, text="New", command=self.file_new, fg=fontColor, bg=colorOfBg)
        self.newButton.grid(row=0, column=0, rowspan=1, sticky="news")
        self.openButton = tk.Button(self.editButtons, text="Open", command=self.file_open, fg=fontColor, background=colorOfBg)
        self.openButton.grid(row=0, column=1, rowspan=1,sticky="ns")
        self.saveButton = tk.Button(self.editButtons, text="Save", command=self.file_save, fg=fontColor, background=colorOfBg)
        self.saveButton.grid(row=0, column=2, rowspan=1, sticky="ns")
        
        self.saveAsButton = tk.Button(self.editButtons, text="SaveAs", command=self.file_save_as, fg=fontColor, background=colorOfBg)
        self.saveAsButton.grid(row=0, column=3, rowspan=1, columnspan=2, sticky="nws")
        self.exitButton = tk.Button(self.editButtons, text="Quit", height=3, command=self.close_window, fg=fontColor, background=colorOfBg)
        self.exitButton.grid(row=0, column=5, rowspan=2, sticky="ns")
        
        self.searchBox = tk.Entry(self.editButtons, width=8, fg=fontColor, bg=colorOfBg)
        self.searchBox.bind("<Return>", self.searchText)
        self.searchBox.bind("<Button-3>", self.searchText)
        tk.Button(self.editButtons, text="Search", command=self.searchText, fg=fontColor, bg=colorOfBg).grid(row=1, column=0, sticky="ew")
        self.searchBox.grid(row=1, column=1, sticky="ew", columnspan=2)
        
        self.lineBox = tk.Entry(self.editButtons, width=3, fg=fontColor, bg=colorOfBg)
        self.lineBox.bind("<Return>", self.gotoLine)
        self.lineBox.bind("<Button-3>", self.gotoLine)
        tk.Button(self.editButtons, text="Line", command=self.gotoLine, fg=fontColor, bg=colorOfBg).grid(row=1, column=3, sticky="w")
        self.lineBox.grid(row=1, column=4, sticky="ew")
        # Spacer
        tk.Label(self.editButtons, text=" ", width=1, fg=fontColor, bg=colorOfBg).grid(row=1, column=6)

        ## CDM8 Emulator Buttons
        # Cludge for Mac button display!!!
        if platform == "darwin":
            self.saveImageButton = tk.Button(buttonBar, text="Save Image ",
                height=3, command=self.saveImage, fg=fontColor, bg=colorOfBg)
        else:
            self.saveImageButton = tk.Button(buttonBar, text="Save Mem\nImage ", 
                height=3, command=self.saveImage, fg=fontColor, bg=colorOfBg)
        
        self.saveImageButton.pack(side=tk.RIGHT)
        self.spacer=tk.Label(buttonBar, text="", width=2, fg=fontColor, bg=colorOfBg)
        self.spacer.pack(side=tk.RIGHT)
        self.speedScale = tk.Scale(buttonBar, from_=3, to=0, orient=tk.HORIZONTAL,
            showvalue=0, width=18, label="Step <--> Fast", fg=fontColor, bg=colorOfBg, activebackground = colorOfBg)
        self.speedScale.set(1)
        self.speedScale.pack(side=tk.RIGHT)
        self.runStopButton = tk.Button(buttonBar, text="Run ", height=3, padx=6,
            command=self.runProg, fg=fontColor, bg=colorOfBg)
        self.runStopButton.pack(side=tk.RIGHT)

        self.runFromFrame = tk.Frame(buttonBar, bg=colorOfBg)
        self.runFromFrame.pack(side=tk.RIGHT)
        self.runFromLabel = tk.Label(self.runFromFrame, text="Run From", fg=fontColor, bg=colorOfBg)
        ...
        style = ttk.Style()
        style.map('TCombobox', fieldbackground=[('readonly', colorOfBg)])
        style.map('TCombobox', selectbackground=[('readonly', colorOfBgBanks)])
        style.map('TCombobox', selectforeground=[('readonly', fontColor)])
        style.map('TCombobox', foreground=[('readonly', fontColor)])
        
        self.runEPSelect = ttk.Combobox(self.runFromFrame, textvariable=self.runFrom,
            state='readonly', width=6, foreground="#D8D8D8", background="#35333D")
        ...
        # Cludge for Mac/OSX
        if platform == "darwin":
            compileButton = tk.Button(buttonBar, text="Compile/Reset", command=self.compileText, height=3, ipadx=6, fg=fontColor, bg=colorOfBg)
        else:
            compileButton = tk.Button(buttonBar, text="Compile\nReset", command=self.compileText, height=3, padx=6, fg=fontColor, bg=colorOfBg)
        compileButton.pack(side=tk.RIGHT)
        
        # Staus/Run time warning/Error pane
        self.statusMsg=tk.Label(buttonBar, text="", width=20, fg="#FF4BFF", bg=colorOfBg, 
        ...
        if amesSession:
            ...
            self.amesFrame=tk.Frame(buttonBar, relief="flat", padx=6,  bg=colorOfBg)
            self.amesFrame.pack(side=tk.LEFT, expand=0, fill=tk.Y)
            self.amesButton = tk.Button(self.amesFrame, text="Start ", command=self.amesStart, fg=fontColor, bg=colorOfBg)#, height=2)
            self.amesButton.grid(row = 1, column=0, sticky="w")#, rowspan=1)
            self.pinVar = tk.IntVar()
            self.pinVar.set("")
            self.pinEntry = tk.Entry(self.amesFrame, show="*", text=self.pinVar, width = 4, fg=fontColor, bg=colorOfBg)
            self.pinEntry.grid(row=1, column=1, padx=2, sticky="w")
            self.pinEntry.bind("<Return>", self.amesStart)
            self.pinLabel = tk.Label(self.amesFrame, text = "AMES          PIN?",  anchor="w", width=18, fg=fontColor, bg=colorOfBg)
        ...
        mainPanel = tk.Frame(self, name='cocoidewin', bg=colorOfBg)#, bg="yellow")
        ...
        self.lntext = tk.Text(mainPanel, width = 4,
                padx = 4, highlightthickness = -1,
                takefocus = 0, bd = 0, background = colorOfTextLeftBg,
                foreground = fontColor, font=self.boldfont,
                yscrollcommand=self.yscroll3, fg=fontColor)
        ...
        self.asstxt = tk.Text(mainPanel, wrap=tk.NONE,font=self.defaulttxtfont,
                                undo=True, yscrollcommand=self.yscroll1,  highlightthickness = -1,
                                autoseparators=True, maxundo=-1, width=40, fg=fontColor, bg=colorOfBg, insertbackground = fontColor)#, height=10,) height=editorHeight,
        ...
        self.mcode_list=tk.Text(mcode_frame, yscrollcommand=self.yscroll2, width=25,
                                wrap=tk.NONE, font=self.defaulttxtfont, fg=fontColor, bg=colorOfBg)#, height=editorHeight)
        ...
        watchPanel = tk.Frame(mainPanel, name="watchpanel", bg=colorOfBg)#, bg="blue")#,  height=80)#,  background="red") #width=570,
        ...
        # And contents
        watchTitle = tk.Label(watchPanel, text="Memory Watches", relief="raised",  padx=0, font=self.boldfont, fg=fontColor, bg=colorOfBg)#, width=51)
        watchTitle.grid(row=0, column=0, columnspan=2, sticky="ew")#pack(side=tk.TOP, fill=tk.X)

        watchHeader = tk.Label(watchPanel, text="Adr:  Label:                  Content ", fg=fontColor, bg=colorOfBg)
        watchHeader.grid(row=1, column=0, columnspan=2, sticky="w")#pack(side=tk.TOP, fill=tk.X, anchor="w")
        
        self.watchList = tk.Text(watchPanel, height=9, font=self.boldfont,  wrap=tk.NONE, fg=fontColor, bg=colorOfBg)#,width=editorWidth-2, text='data: OE: "Hello there"')
        self.watchList.bind("<Key>", lambda e: "break") # Disable editing
        self.watchList.grid(row=2, column=0, columnspan=2, sticky="nsew")
        watchScrollY = ttk.Scrollbar(watchPanel, orient=tk.VERTICAL, command=self.watchList.yview)
        self.watchList.config(yscrollcommand=watchScrollY.set)
        watchScrollY.grid(row=2, column=1, sticky="nse")
        watchScrollX = ttk.Scrollbar(watchPanel, orient=tk.HORIZONTAL, command=self.watchList.xview)
        ...
        self.regPanel = tk.Frame(mainPanel, name="regpanel",  height=80, padx=0, relief="sunken", bd=2, bg=colorOfBg)#,  background="blue") #width=300,
        self.regPanel.grid(row=2, column=3, sticky="nsew")#pack(side=tk.LEFT, fill=tk.Y)
        regTitle = tk.Label(self.regPanel, text="Registers", font=self.boldfont, fg=fontColor, bg=colorOfBg)
        regTitle.grid(row=0, column=1, columnspan=2)
        
        # Program counter
        self.pcLab = tk.Label(self.regPanel, text="^ PC  ", width=6, font=self.boldfont, padx=3, fg=fontColor, bg=colorOfBg)
        self.pcLab.grid(row=1, column=0, sticky="w")#, columnspan=2)
        self.pcLabVal = tk.Label(self.regPanel, text="00",width=6, relief="sunken", padx=3, font=self.defaulttxtfont, fg=fontColor, bg=colorOfBg)
        self.pcLabVal.grid(row=2, column=0, sticky="w")

        # PS register (CVZN etc.)
        
        self.CVZN_Lab = tk.Label(self.regPanel, text="PS: I Page CVZN",width=15, font=self.boldfont, fg=fontColor, bg=colorOfBgBanks)
        self.CVZN_Lab.grid(row=1, column=1, columnspan=2)#,sticky="e")#, columnspan=2)
        self.CVZN_Val = tk.Label(self.regPanel, text="0 000 0000", width=15, relief="sunken", font=self.defaulttxtfont, fg=fontColor, bg=colorOfBg)
        self.CVZN_Val.grid(row=2, column=1, columnspan=2)#, sticky="e")#, columnspan=2)

        # Stack Pointer
        self.spLab = tk.Label(self.regPanel, text=" SP  ",width=7, font=self.boldfont, fg=fontColor, bg=colorOfBg)
        self.spLab.grid(row=1, column=3)#, columnspan=2)
        self.spVal = tk.Label(self.regPanel, text="00", width=7, relief="sunken", font=self.defaulttxtfont, fg=fontColor, bg=colorOfBg)
        self.spVal.grid(row=2, column=3)#, columnspan=2)


        spacer1= tk.Label(self.regPanel, text="", bg=colorOfBg)#, height=1)
        ...
        for index in range(4):
            self.regLabs[index] = tk.Label(self.regPanel, text="r"+str(index), width=8, padx=5, fg="#5F67DB", font=self.boldfont, bg=colorOfBg)
            self.regLabs[index].grid(row=4, column=index, sticky="n")
            self.regHexs[index] = tk.Label(self.regPanel, text="0x00", width=8,  bg=colorOfBg, relief="sunken",font=self.defaulttxtfont)
            self.regHexs[index].grid(row=5, column=index, sticky="n")
            self.regStrs[index] = tk.Label(self.regPanel, text="NUL", width=8,  bg=colorOfBg, relief="sunken",font=self.defaulttxtfont)
            self.regStrs[index].grid(row=6, column=index, sticky="n")
            self.regDecs[index] = tk.Label(self.regPanel, text="+000 000", width=8,  bg=colorOfBg, relief="sunken",font=self.defaulttxtfont)
            self.regDecs[index].grid(row=7, column=index, sticky="n")
            self.regBins[index] = tk.Label(self.regPanel, text="00000000", width=8,  bg=colorOfBg, relief="sunken",font=self.defaulttxtfont)
            self.regBins[index].grid(row=8, column=index, sticky="n")

        ### Create the machine panel
        self.mcPanel = tk.Frame(mainPanel, bg=colorOfBg, highlightbackground=colorOfBorders, highlightthickness=4)#, bg="blue")
        ...
        self.memPageFrame = tk.Frame(self.mcPanel, bg=colorOfBg)
        ...
        self.pageLabel = tk.Label(self.memPageFrame, text="Memory: Page:", fg=fontColor, bg=colorOfBg)
        self.pageLabel.pack(side=tk.LEFT)
        self.pageRadButtons = []
        for n in range(8):
            self.pageRadButtons.append(tk.Radiobutton(self.memPageFrame,fg=fontColor, bg=colorOfBg, text=str(n),
        ...
        for n in range(len(self.mem_frame)):
            self.mem_frame[n] = tk.Frame(self.emuNb, bg=colorOfBg)
        ...
        # I/O Title Header
        self.IOHeader = tk.Frame(self.mcPanel, borderwidth=1, bg=colorOfBg, highlightbackground=colorOfBorders)
        #self.IOHeader.pack(side=tk.TOP, fill=tk.X, padx=5, pady=3)
        self.IOHeader.grid(row=2, column=0)
        self.IOAdrLabel =  tk.Label(self.IOHeader, text="Addr.", fg=fontColor, bg=colorOfBg)
        self.IOAdrLabel.pack(side=tk.LEFT)
        self.IOLabel = tk.Label(self.IOHeader, font=self.boldfont, text="               I/O Ports", fg=fontColor, bg=colorOfBg)
        ...
        self.portVarLabel = tk.Button(self.IOHeader, text="+", command=self.addport, width=1, fg=fontColor, bg=colorOfBg)
        ...
        self.IOPanel = tk.Frame(self.mcPanel, bg=colorOfBg)#, bg="red")#.pack(fill=tk.BOTH, expand=1)
        ...
        self.IOcanvas = tk.Canvas(self.IOPanel, yscrollcommand=self.IOscroll.set, bg=colorOfBg)# bg="yellow")
        ...
        self.IOFrame = tk.Frame(self.IOcanvas, bg=colorOfBg)#, bg="purple")
        ...
        try:
            if (lastOpendedFile != '') and (os.path.exists(lastOpendedFile) and os.path.getsize(lastOpendedFile) > 0):
                self.file_open_last(filepath = lastOpendedFile)
    ...
    def amesStart(self, event=None):
        ...
        if errormsg:# or self.timeleft <= 0:
            ...
            self.amesButton.config(text="Submit", state=tk.NORMAL, command=self.amesSubmit, fg=fontColor, bg=colorOfBg)
    ...
    def amesClockUpdate(self, timeleft=None):
        ...
        if  self.amesRunning and (self.timeleft > 0 or self.timeleft < -11): # Still time left?
            ...
            self.pinLabel.config(text = "AMES <"+str(int(self.timeleft//60)+1)+" MIN LEFT", fg=fontColor, bg=colorOfBg)
    ...
    def bindKeys(self):
        ...
        self.asstxt.bind("<Control-a>", self.select_all)
        self.asstxt.bind("<Control-A>", self.select_all)
        self.asstxt.unbind("<Control-d>")
        self.asstxt.unbind("<Control-D>")
        self.asstxt.bind("<Control-d>", self.cpyone)
        self.asstxt.bind("<Control-D>", self.cpyone)
        self.asstxt.bind("<Control-/>", self.commentHighLight)
    ...
    def unbindKeys(self):
        ...
        self.asstxt.bind("<Control-d>", None)
        self.asstxt.bind("<Control-D>", None)
        self.asstxt.bind("<Control-a>", None)
    ...
    def initMemDisplay(self, page=None):
        ...
        for n in range(len(self.mem_frame)):
            gridpos=1
            for col in range(16):
                tk.Label(self.mem_frame[n], text=self.Emu.hx(col)[1]+" ", font=self.boldfont, fg=fontColor, bg=colorOfBg).grid(row=0, column=gridpos, padx=2, pady=2, )
                tk.Label(self.mem_frame[n], text=self.Emu.hx(col)[1]+" ", font=self.boldfont, fg=fontColor, bg=colorOfBg).grid(row=gridpos, column=0, padx=2, pady=2)
                gridpos += 1
        ...
        for n in range(len(self.Emu.memory[self.Emu.curPage])):
            for gridy in range(1,17):
                for gridx in range(1, 17):
                    #print(n, "*", index, end=":")
                    self.memLabel[index] = tk.Label(self.mem_frame[n], font=self.defaulttxtfont, text="00", width=2, fg=fontColor, bg=colorOfBg)#, font=self.smallfont)
    ...
    def dispCVZN(self):
        ...
        self.CVZN_Val.config(text= "    "+intStr+" "+pageStr+"  "+CVZNstr, fg=fontColor, bg=colorOfBg)
        return
    ...
    def dispAllMemory(self, page=None):#?? 
        ...
        for n in range(numbanks): # All banks in in current CDM8 memory page
            ...
            for memval in self.Emu.memory[curPage][n]:#self.Emu.curPage][n]: # For each bank
                self.memLabel[index].config(text=self.Emu.hx(memval), fg=fontColor, bg=colorOfBgBanks)
                ...
                if self.memChanged:
                    ...
                    if index == 0 or self.Emu.PC==self.runDict[self.runFrom.get()] == 0: # Except at start of run
                        self.memLabel[index].config(fg=fontColor)
                        ...
                        for addr in self.memChanged:
                            if (index % 256) == addr:# Highlight if memory cell has changed
                                ...
                                self.memLabel[index].config(fg=ChangedBank)
                    ...
                    if  self.arch[curPage]=="vn" and (self.Emu.PC in self.memChanged): #(n < self.Emu.datamem[curPage] or
                        ...
                        txtaddr = self.mcode_list.search(self.Emu.hx(self.Emu.PC)+":", "1.0", tk.END, fg = fontColor, bg = colorOfBgBanks)
                        if txtaddr:
                            self.mcode_list.tag_add("smod", "%s linestart+3c" % txtaddr, "%s lineend+1c" % txtaddr, fg=fontColor) # add tag to k
                            self.mcode_list.tag_config("smod", font=self.boldfont, foreground=fontColor)
                else:
                    self.memLabel[index].config(fg=fontColor, text=self.Emu.hx(memval), bg=colorOfBgBanks)
                        ...
                        for adr in port.getOPadr():
                            ...
                            if n == self.Emu.datamem[0] and adr == indadr: # Only page 0 has IO
                                if adr in self.memChanged:
                                    fgcolour=fontColor#cf.chMemColour
                                else: 
                                    fgcolour=fontColor
                                self.memLabel[index].config(fg=fontColor, bg=colorOfBgBanks, text=self.Emu.hx(memval))
                        for adr in port.getIPadr():
                            if n == self.Emu.datamem[0] and adr == index - (self.Emu.datamem[0] *256):
                                self.memLabel[index].config(bg=colorOfBgBanks, fg=fontColor, text=("%02X" % port.portIPvals[adr]))#self.Emu.hx(memval))
    ...
    def dispPC(self):
        ...
        if self.prevPC in self.Emu.BP:
            self.memLabel[self.prevPC].config( bg=colorOfBg)
        else:
            self.memLabel[self.prevPC].config(bg="#35333D")
        self.prevPC=self.Emu.PC
        self.pcLabVal.config(text=self.Emu.hx(self.Emu.PC), fg=fontColor, bg='#B57911')
        self.memLabel[self.Emu.PC].config(bg='#B57911')
    ...
    def dispRegs(self):
        for index in range(4):
            self.regHexs[index].config(text="0x"+self.Emu.hx(self.Emu.regs[index]), fg=fontColor, bg=colorOfBg) # hex row
            self.regStrs[index].config(text=self.Emu.convert(2,self.Emu.regs[index]), fg=fontColor, bg=colorOfBg) # Char row

            self.regDecs[index].config(text="%+04d" % int(self.Emu.convert(1,self.Emu.regs[index]))+ " %03d" % self.Emu.regs[index], fg=fontColor, bg=colorOfBg) # dec row
            #print("*"+convert(1,Regs[k]))# debug
            self.regBins[index].config(text=format(self.Emu.regs[index],"08b"), fg=fontColor, bg=colorOfBg) # Bin row
        return
    ...
    def dispSP (self):
        ...
        self.memLabel[self.prevSP+self.Emu.datamem[stackpage]*256].config(fg=fontColor, bg=colorOfBg)
        if self.Emu.SP[stackpage] != 0: #self.runFrom.get():
            self.memLabel[self.Emu.SP[stackpage]+self.Emu.datamem[stackpage]*256].config(fg=fontColor, bg=colorOfBg)
    ...
    def runProg(self, event=None):
        ...
        self.updateDisp()
        self.running=False
        self.runStopButton.config(text="Run ", fg=fontColor, activeforeground="black" )
    ...
    def highlighter(self,  event=None):
        ...
        self.asstxt.tag_config("all", foreground=fontColor, font=self.defaulttxtfont)#font=self.boldfont)
            ...
            while startIndex:
                ...
                if startIndex:
                    ...
                    if not self.asstxt.search(cf.commentprefix, lineStart+" linestart", lineStart):
                        self.asstxt.tag_add(word, startIndex, endIndex) # add tag to k
                        self.asstxt.tag_config(word, foreground=markColor, font=self.boldfont)      # and color it with v
                    startIndex = endIndex # reset startIndex to continue searching
        return
    ...
    def popmenu(self, event=None):
        menu = tk.Menu(self.master,tearoff=0, bg = colorOfBg, fg = fontColor, bd = -1)
    ...
    def file_new(self, event=None):
        ...
        if result != None: #None => Aborted or Save cancelled, False => Discarded, True = Saved or Not modified
            ...
            self.resetEmu()
        return "break"
    def file_open_last(self,filepath=None):
        fileContents=""
        f = open(filepath)
        fileContents = f.read()
        # Set current text to file contents
    ...
    def file_open(self, event=None, filepath=None):
        ...
        if result != None: #None => Aborted or Save cancelled, False => Discarded, True = Saved or Not modified
            if filepath == None:
                ...
                smartWriterOfTheme(colorOfBg , colorOfBgBanks , colorOfTextLeftBg , colorOfBorders , ChangedBank , markColor , fontColor , colorOfCommands , colorOfMarks , colorOfOperators, filepath)
            fileContents=""
    ...
    def cpyone(self, event = None):
        line_no = event.widget.index(float(int(float(self.asstxt.index("insert")))))
        line_end = event.widget.index(float(int(float(self.asstxt.index("insert"))))+1)
        try:
            self.asstxt.clipboard_clear()
            self.cliptext = self.asstxt.get(line_no,line_end) 
            self.asstxt.insert(line_end,self.cliptext)
            self.changed = True
            self.highlighter()
        except tk.TclError:
            pass
    ...
    def commentHighLight(self, event=None):
        try:
            try:
                line_no  = (math.floor(float(self.asstxt.index("sel.first"))))
                line_end = (math.ceil(float(self.asstxt.index("sel.last"))))
            except tk.TclError:
                line_no = (math.floor(float(self.asstxt.index("insert"))))
                line_end = line_no+1
            hashFlag = 1
            for i in range(line_no, line_end):
                if(self.asstxt.get(float(i)) == "#"):
                    hashFlag = 0
                    break;
            for i in range(line_no, line_end):
                if(hashFlag):
                    self.asstxt.insert(float(i), "#")
                else:
                    if(self.asstxt.get(float(i)) == "#"):
                        self.asstxt.delete(float(i))
            #self.cliptext = self.asstxt.get(line_no,line_end).replace("\n", "\n#")
            #self.asstxt.delete(line_no,line_end)
            #self.asstxt.insert(line_no,self.cliptext)
            self.changed = True
            self.highlighter()
        except tk.TclError:
            pass
    ...
    def themeDark(self, event=None):
        smartWriterOfTheme('#35333D' , '#465362' , '#393740' , '#28262D' , '#DC5EAB' , '#8862CB' , '#D8D8D8' , '#32A5F2' , '#0BA242' , '#B232F2',lastOpendedFile)
        python = sys.executable
        os.execl(python, python, * sys.argv)
        
    def themeLight(self, event=None):
        smartWriterOfTheme('#DEDEE8' , '#A2A2B0' , '#A2A2B0' , '#A2A2B0' , '#5FC425' , '#0490EE' , '#101D25' , '#0022FF' , '#008A29' , '#FF004D',lastOpendedFile)
        python = sys.executable
        os.execl(python, python, * sys.argv)

    def themePink(self, event=None):
        smartWriterOfTheme('#EB75C4', '#BF4195' , '#BF4195' , '#DB2CA1' , '#03F29A' , '#F2034A' , '#303C54' , '#0022FF' , '#008A29' , '#FF004D',lastOpendedFile)
        python = sys.executable
        os.execl(python, python, * sys.argv)
    
    def myThemeCreate(self, event=None):
        smartReaderOfTheme('Themes\mytheme.txt')
        smartWriterOfTheme(colorOfBg , colorOfBgBanks , colorOfTextLeftBg , colorOfBorders , ChangedBank , markColor , fontColor , colorOfCommands , colorOfMarks , colorOfOperators, lastOpendedFile)
        python = sys.executable
        os.execl(python, python, * sys.argv)
        ...
        #self.myTheme = tk.Tk()
        #self.myTheme.title("Цветовая палитра")
        
        #self.btnBG = tk.Button(self.myTheme, text="Выберите цвет", command=self.bgColor)
        #self.btnBG.pack()
        
        #self.myTheme.mainloop
